...
$('document').ready(function() {
	...
	$(window).on('action:composer.loaded', function(err, data) {
		...
		var item = $('<button type="button" class="btn btn-info dropdown-toggle" data-toggle="dropdown"><span class="caret"></span></button><ul class="dropdown-menu pull-right" role="menu"><li><a href="#" data-switch-action="post"><i class="fa fa-fw fa-question-circle"></i> 提问题</a></li></ul>');
		...
		if (
			...
			$('.composer-submit').attr('data-action', 'post').html('<i class="fa fa-fw fa-question-circle"></i> 提问题</a>');
			$(window).off('action:composer.topics.post').one('action:composer.topics.post', function(ev, data) {
				callToggleQuestion(data.data.tid, false);
			});
		} else {
	...
	function callToggleQuestion(tid, refresh) {
		socket.emit('plugins.QandA.toggleQuestionStatus', {tid: tid}, function(err, data) {
			app.alertSuccess(data.isQuestion ? '成功设为问题' : '已恢复为普通主题');
			if (refresh) {
				ajaxify.refresh();
			}
		});
	...
	function toggleSolved() {
		...
		socket.emit('plugins.QandA.toggleSolved', {tid: tid}, function(err, data) {
			app.alertSuccess(data.isSolved ? '问题设为已解决' : '问题设为未解决');
			ajaxify.refresh();
		});
	}
	...
	function markPostAsAnswer() {
		...
		socket.emit('plugins.QandA.toggleSolved', {tid: tid, pid: pid}, function(err, data) {
			app.alertSuccess(data.isSolved ? '此帖子已标记为正确的答案' : '主题已标记为未解决');
			ajaxify.refresh();
		});
	}
